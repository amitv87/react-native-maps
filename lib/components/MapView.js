...
const propTypes = {
  ...
  onCameraMoveStarted: PropTypes.func,
};
...
class MapView extends React.Component {
  ...
  moveCameraWithBrg(latlng, zoom, tilt, brg){
    this._runCommand('moveCameraWithBrg', [latlng, zoom, tilt ? tilt : 0, brg ? brg : 0]);
  }
  ...
  clearPoly(key){
    this._runCommand('clearPoly', [key]);
  }

  createPoly(key, color, width){
    this._runCommand('createPoly', [key, color, width]);
  }

  addPointToPoly(key, coordinate){
    this._runCommand('addPointToPoly', [key, coordinate]);
  }

  addPointsToPoly(key, coordinates){
    this._runCommand('addPointsToPoly', [key, coordinates]);
  }

  removePointsFromPoly(key, count){
    this._runCommand('removePointsFromPoly', [key, count]);
  }

  setMapInteractive(flag){
    this._runCommand('setMapInteractive', [flag]);
  }
